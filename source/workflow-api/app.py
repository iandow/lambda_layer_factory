...
#WORKFLOW_EXECUTION_TABLE_NAME = "mas-workflowWorkflowExecution"
...
ANALYSIS_STAGE_QUEUE_ARN = "arn:aws:sqs:us-east-1:526662735483:Analysis"
PREPROCESS_STAGE_QUEUE_ARN = "arn:aws:sqs:us-east-1:526662735483:Preprocess"
POSTPROCESS_STAGE_QUEUE_ARN = "arn:aws:sqs:us-east-1:526662735483:Postprocess"


#ANALYSIS_STAGE_QUEUE_URL = "https://sqs.us-east-1.amazonaws.com/526662735483/Analysis"
#PREPROCESS_STAGE_QUEUE_URL = "https://sqs.us-east-1.amazonaws.com/526662735483/Analysis"
#POSTPROCESS_STAGE_QUEUE_URL = "https://sqs.us-east-1.amazonaws.com/526662735483/Analysis"

START_STAGE_LAMBDA_ARN = ""
END_STAGE_LAMBDA_ARN = ""
...
EXECUTE_STAGE_QUEUE_URL = os.environ["EXECUTE_STAGE_QUEUE"]
...
DEFAULT_WORKFLOW_SQS = {
    ...
    "Stages": {
        "Preprocess": {
            "Type": "NestedQueue",
            "Resources": {
                "Queue": PREPROCESS_STAGE_QUEUE_URL,
                "StateMachine": ANALYSIS_STAGE_STATE_MACHINE_ARN
            },
            "Next": "Analysis"
        },
        "Analysis": {
            "Type": "NestedQueue",
            "Resource": EXECUTE_STAGE_QUEUE_URL,
            "Next": "Postprocess"
        },
        "Postprocess": {
            "Type": "NestedQueue",
            "Resource": POSTPROCESS_STAGE_QUEUE_URL,
            "End": True
        }
    }
}
...
def create_stage():
    ...
    stage_asl = {
        "startAt": "getStageForExecution",
        "States": {
            "getStageForExecution": {
...
# Build a workflow stage
# In:
#    {
#    "name":"Default",
#    "stageStateMachineArn",
#    "stageConfiguration": {
#        {
#        Preprocess": {
#            Operations: {
    ...
    try:
        ...
        for stage in workflow["Stages"]:
            ...
            stageDefinition = {
                ...
                "stateMachineQueue": ANALYSIS_STAGE_QUEUE_ARN
            }
            stage["stateMachineArn"] = stageDefinition['stateMachineArn']
            stage["stateMachineQueue"] = stageDefinition['stateMachineQueue']
...
def start_next_stage_execution(trigger, workflow_execution):
    ...
    try:
        ...
        if current_stage != None and "End" in workflow_execution["workflow"]["Stages"][current_stage] and workflow_execution["workflow"]["Stages"][current_stage]["End"] == True:
                
                workflow_execution["current_stage"] = "End"
                workflow_execution["status"] = WORKFLOW_STATUS_COMPLETE
        else:
            if current_stage == None:
                current_stage = workflow_execution["current_stage"] = workflow_execution["workflow"]["StartAt"]
